...
export class ScheduleRepositoryImpl implements ScheduleRepository {
  ...
  async findRecordsByTeacherFullName(
    fullName: string
  ): Promise<ScheduleRecordEntity[]> {
    const models = await ScheduleRecordModel.find({
      teachers: { $in: [fullName] },
    });
    if (!models) throw new Error("Не удалось найти записи");
    return models.map((model) =>
      this.scheduleRecordDataBaseConverter.toEntity(model)
    );
  }

  async findRecordsByGroupNumber(
    number: number
  ): Promise<ScheduleRecordEntity[]> {
    const models = await ScheduleRecordModel.find({
  ...
  ): Promise<ScheduleEntryEntity> {
    const newScheduleEntryModel =
      this.scheduleEntryDataBaseConverter.toModel(entry);
    const savedModel = await new ScheduleEntryModel(
      newScheduleEntryModel
    ).save();
    if (!savedModel) {
  ...
  async addScheduleRecord(record: ScheduleRecordEntity): Promise<boolean> {
    const newScheduleRecord =
      this.scheduleRecordDataBaseConverter.toModel(record);
  ...
  async findRecordsByEntryId(id: string): Promise<ScheduleRecordEntity[]> {
    const models = await ScheduleRecordModel.find({ scheduleEntryId: id });
    return models.map((model) =>
      this.scheduleRecordDataBaseConverter.toEntity(model)
    );
  }
